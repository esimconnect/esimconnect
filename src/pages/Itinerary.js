...
export default function Itinerary() {
  ...
  const saveItinerary = async () => {
    ...
    if (error) { setSavedStatus('error'); }
    else { setSavedStatus('saved'); }
  ...
  const saveAsPDF = () => {
    ...
    style.innerHTML = `@media print { body * { visibility: hidden; } #itinerary-print-area, #itinerary-print-area * { visibility: visible; } #itinerary-print-area { position: absolute; left: 0; top: 0; width: 100%; } .no-print { display: none !important; } .day-card { page-break-inside: avoid; margin-bottom: 24px; } }`;
  ...
  useEffect(() => {
    const handleVisibility = async () => {
      if (document.visibilityState === 'visible') {
        ...
        if (currentUser) { setUser(currentUser); setShowGateModal(false); }
      }
    };
  ...
  const init = async () => {
    ...
    if (user) {
      ...
      const { data: orders } = await supabase.from('orders').select('id').eq('user_id', user.id).eq('status', 'completed').limit(1);
      if (orders && orders.length > 0) setIsPlanBuyer(true);
      const { data: logs } = await supabase.from('usage_logs').select('id').eq('user_id', user.id).eq('action', 'itinerary_search');
  ...
  const detectLatestTrip = async (userId) => {
    const { data: esimRows } = await supabase.from('esims').select('country_name, country_flag, activated_at, validity_days, created_at').eq('user_id', userId).order('created_at', { ascending: false }).limit(1);
    const esimData = esimRows?.[0] || null;
    const { data: orderRows } = await supabase.from('orders').select('*, countries(name, flag_emoji)').eq('user_id', userId).eq('payment_status', 'paid').order('created_at', { ascending: false }).limit(1);
    const orderData = orderRows?.[0] || null;
    let trip = null;
    if (esimData && orderData) {
      trip = new Date(esimData.created_at) >= new Date(orderData.created_at) ? buildTripFromEsim(esimData) : buildTripFromOrder(orderData);
    } else if (esimData) { trip = buildTripFromEsim(esimData); }
    else if (orderData) { trip = buildTripFromOrder(orderData); }
    setDetectedTrip(trip);
  };

  const buildTripFromEsim = (e) => ({ destination: e.country_name, flag: e.country_flag || '🌍', startDate: e.activated_at ? new Date(e.activated_at).toISOString().split('T')[0] : new Date().toISOString().split('T')[0], duration: e.validity_days || 7, source: 'eSIM' });
  const buildTripFromOrder = (o) => ({ destination: o.countries?.name || 'Your destination', flag: o.countries?.flag_emoji || '🌍', startDate: o.paid_at ? new Date(o.paid_at).toISOString().split('T')[0] : new Date().toISOString().split('T')[0], duration: 7, source: 'Order' });
  ...
  const getRouteMarkers = (plan) => {
    ...
    plan.days?.forEach(day => { day.stops?.forEach(stop => { if (stop.lat && stop.lng) markers.push({ ...stop, day: day.day }); }); });
    return markers;
  };
  ...
  const searchPlaces = async () => {
    ...
    try {
      ...
      const res = await fetch(`https://nominatim.openstreetmap.org/search?q=${q}+${dest}&format=json&limit=5&addressdetails=1`, { headers: { 'Accept-Language': 'en' } });
      const data = await res.json();
      setSearchResults(data.map(r => ({ name: r.name || r.display_name.split(',')[0], address: r.display_name, lat: parseFloat(r.lat), lng: parseFloat(r.lon), category: manualCategory })));
    } catch (e) { setSearchResults([]); }
    setSearching(false);
  };

  const addToBasket = (place) => { const id = `manual_${Date.now()}`; setBasket(prev => [...prev, { ...place, id }]); setSearchResults([]); setSearchQuery(''); };
  const addManualToBasket = () => { if (!manualName.trim()) return; const id = `manual_${Date.now()}`; setBasket(prev => [...prev, { id, name: manualName, address: manualAddress, category: manualCategory, lat: null, lng: null }]); setManualName(''); setManualAddress(''); };
  ...
  const buildManualRoute = async () => {
    ...
    try {
      const response = await fetch(CLAUDE_API, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ model: MODEL, max_tokens: 4000, messages: [{ role: 'user', content: prompt }] }) });
      const data = await response.json();
      const text = data.content?.map(b => b.text || '').join('');
      const clean = text.replace(/```json|```/g, '').trim();
      const parsed = JSON.parse(clean);
      parsed.days = parsed.days.map(day => ({ ...day, stops: day.stops.map(stop => { if (!stop.lat || !stop.lng) { const original = basket.find(b => b.name === stop.name); if (original) { stop.lat = original.lat; stop.lng = original.lng; } } return stop; }) }));
  ...
  const inputStyle = { background: 'rgba(255,255,255,0.06)', border: '1px solid var(--border)', borderRadius: '10px', padding: '12px 16px', fontSize: '15px', color: 'var(--text)', outline: 'none', width: '100%', boxSizing: 'border-box' };
  const labelStyle = { fontSize: '13px', fontWeight: 600, color: 'var(--muted)' };
  const cardStyle = (sel) => ({ display: 'flex', alignItems: 'flex-start', gap: '10px', cursor: 'pointer', padding: '12px 14px', borderRadius: '14px', background: sel ? 'rgba(0,200,255,0.08)' : 'rgba(255,255,255,0.03)', border: `1px solid ${sel ? 'rgba(0,200,255,0.3)' : 'rgba(255,255,255,0.07)'}`, transition: 'all 0.15s' });

  if (loading) return <div className={styles.loadingPage}><div className={styles.spinner}></div></div>;

  const GateModal = () => (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.75)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: '20px' }}>
      <div style={{ background: '#0d1117', border: '1px solid rgba(0,200,255,0.3)', borderRadius: '20px', padding: '36px', maxWidth: '420px', width: '100%', textAlign: 'center' }}>
        ...
        <div style={{ background: 'rgba(0,200,255,0.05)', border: '1px solid rgba(0,200,255,0.15)', borderRadius: '14px', padding: '16px', marginBottom: '24px', textAlign: 'left' }}>
          {gateReason === 'guest' && (<div style={{ marginBottom: '10px', fontSize: '13px', display: 'flex', gap: '10px' }}><span>🆓</span><span><strong>Register free</strong> — get 5 searches + save trips</span></div>)}
          <div style={{ marginBottom: '10px', fontSize: '13px', display: 'flex', gap: '10px' }}><span>♾️</span><span><strong>Buy any data plan</strong> — unlimited searches forever</span></div>
          <div style={{ marginBottom: '10px', fontSize: '13px', display: 'flex', gap: '10px' }}><span>📱</span><span><strong>Re-download QR codes</strong> anytime from My Purchases</span></div>
          <div style={{ fontSize: '13px', display: 'flex', gap: '10px' }}><span>🧾</span><span><strong>Full order history</strong> — all eSIMs in one place</span></div>
        </div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
          <button onClick={() => { setShowGateModal(false); navigate('/plans'); }} style={{ background: 'linear-gradient(135deg, var(--accent), var(--accent2))', border: 'none', borderRadius: '12px', padding: '14px', color: '#000', fontWeight: 800, fontSize: '15px', cursor: 'pointer' }}>Buy a Data Plan — Unlock Unlimited →</button>
          {gateReason === 'guest' && (<button onClick={() => { setShowGateModal(false); navigate('/register'); }} style={{ background: 'rgba(255,255,255,0.07)', border: '1px solid rgba(255,255,255,0.15)', borderRadius: '12px', padding: '14px', color: 'inherit', fontWeight: 700, fontSize: '14px', cursor: 'pointer' }}>Register Free — Get 5 Searches</button>)}
          {gateReason === 'guest' && (<button onClick={() => { window.open('/login?from=itinerary', '_blank'); setShowGateModal(false); }} style={{ background: 'none', border: 'none', color: 'var(--accent)', fontSize: '13px', cursor: 'pointer', padding: '4px', textDecoration: 'underline' }}>{t('auth_have_account')} {t('auth_login')}</button>)}
          <button onClick={() => setShowGateModal(false)} style={{ background: 'none', border: 'none', color: 'var(--muted)', fontSize: '13px', cursor: 'pointer', padding: '8px' }}>{t('cancel')}</button>
        </div>
      </div>
    </div>
  );
      ...
      <main className={styles.main}>
          ...
          <div style={{ maxWidth: '680px' }}>
            <div style={{ marginBottom: '24px' }}>
              <h2 style={{ fontFamily: 'var(--font-head)', fontSize: '20px', fontWeight: 900, marginBottom: '6px' }}>
                {t('itin_interests')} {activeTrip?.flag}
              </h2>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: '10px' }}>
                {STANDARD_CATEGORIES.map(cat => (
                  <label key={cat.id} style={cardStyle(selectedInterests[cat.id])}>
                    <input type="checkbox" checked={!!selectedInterests[cat.id]} onChange={() => toggleInterest(cat.id)} style={{ marginTop: '2px', accentColor: 'var(--accent)', width: '15px', height: '15px', flexShrink: 0 }} />
                    <div>
            ...
            <div style={{ marginBottom: '28px' }}>
              <div style={{ fontSize: '12px', fontWeight: 700, color: 'var(--accent)', textTransform: 'uppercase', letterSpacing: '0.5px', marginBottom: '12px' }}>
                {loadingExtra ? `⏳ ${t('loading')}` : `✨ Unique to ${activeTrip?.destination}`}
              </div>
        ...
        {generating && (
          <div style={{ textAlign: 'center', padding: '60px 24px' }}>
            ...
            <p style={{ color: 'var(--muted)', fontSize: '15px' }}>{t('loading')} {activeTrip?.destination}...</p>
          </div>
        )}
        ...
        {suggestions && !routedPlan && !routing && (() => {
          ...
          <div style={{ maxWidth: '760px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '16px', flexWrap: 'wrap', gap: '12px' }}>
              ...
              <button onClick={() => { setSuggestions(null); setShowInterests(true); }} style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid var(--border)', color: 'var(--muted)', borderRadius: '10px', padding: '8px 16px', fontSize: '13px', fontWeight: 600, cursor: 'pointer' }}>← {t('back')}</button>
            </div>
            ...
            <AddPlaceSearch activeTrip={activeTrip} onAdd={(place) => { setSuggestions(prev => ({ ...prev, places: [...(prev.places || []), place] })); setSelected(prev => ({ ...prev, [place.id]: true })); }} />
            ...
            <div style={{ marginTop: '24px', background: 'rgba(10,15,26,0.95)', border: '1px solid var(--border)', borderRadius: '16px', padding: '20px 24px', backdropFilter: 'blur(20px)' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px', flexWrap: 'wrap', gap: '12px' }}>
                <div style={{ fontSize: '14px', color: 'var(--muted)' }}>
                  <span style={{ color: 'var(--text)', fontWeight: 700 }}>{selectedCount}</span> places selected · Claude will cluster & sort into <span style={{ color: 'var(--text)', fontWeight: 700 }}>{activeTrip && activeTrip.duration}</span> {t('days')}
                </div>
                <button onClick={savePickedPlaces} disabled={selectedCount === 0} style={{ background: selectedCount === 0 ? 'rgba(255,255,255,0.05)' : 'rgba(0,200,255,0.1)', border: '1px solid ' + (selectedCount === 0 ? 'rgba(255,255,255,0.1)' : 'rgba(0,200,255,0.3)'), color: selectedCount === 0 ? 'var(--muted)' : 'var(--accent)', borderRadius: '10px', padding: '8px 18px', fontWeight: 700, fontSize: '13px', cursor: selectedCount === 0 ? 'not-allowed' : 'pointer' }}>
                  {savingList === 'saving' ? `⏳ ${t('loading')}` : savingList === 'saved' ? `✅ ${t('itin_saved')}!` : savingList === 'error' ? `❌ ${t('error')}` : `💾 ${t('save')}`}
                </button>
              </div>
              <div style={{ borderTop: '1px solid rgba(255,255,255,0.08)', paddingTop: '16px', marginBottom: '16px' }}>
                ...
                <input type="text" placeholder="e.g. Marina Bay Sands, Singapore" value={globalStartPoint} onChange={e => setGlobalStartPoint(e.target.value)} style={{ width: '100%', background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.12)', borderRadius: '10px', padding: '10px 14px', color: 'inherit', fontSize: '14px', boxSizing: 'border-box', marginBottom: '10px' }} />
                <div onClick={() => setShowDayOverrides(!showDayOverrides)} style={{ fontSize: '12px', color: 'var(--accent)', cursor: 'pointer', display: 'inline-flex', alignItems: 'center', gap: '4px' }}>
                  ...
                  <div style={{ marginTop: '10px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
                    {Array.from({ length: parseInt(activeTrip?.duration || 1) }, (_, i) => (
                      <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                        ...
                        <input type="text" placeholder={`Override for Day ${i+1}`} value={dayStartPoints[i+1] || ''} onChange={e => setDayStartPoints(prev => ({ ...prev, [i+1]: e.target.value }))} style={{ flex: 1, background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '8px', padding: '8px 12px', color: 'inherit', fontSize: '13px' }} />
                      </div>
                    ))}
                  </div>
                )}
              </div>
              <button onClick={buildRoute} disabled={selectedCount === 0} style={{ width: '100%', background: selectedCount === 0 ? 'rgba(255,255,255,0.1)' : 'linear-gradient(135deg, var(--accent), var(--accent2))', color: selectedCount === 0 ? 'var(--muted)' : '#000', border: 'none', borderRadius: '12px', padding: '14px 24px', fontWeight: 800, fontSize: '15px', fontFamily: 'var(--font-head)', cursor: selectedCount === 0 ? 'not-allowed' : 'pointer' }}>
                {selectedCount === 0 ? 'Select places to build your route' : `${t('itin_generate')} → (${selectedCount} places)`}
              </button>
            </div>
          </div>
          );
        ...
        {/* Routing / Building spinners */}
        {routing && (<div style={{ textAlign: 'center', padding: '60px 24px' }}><div className={styles.spinner} style={{ margin: '0 auto 20px' }}></div><p style={{ color: 'var(--muted)', fontSize: '15px' }}>{t('loading')} {activeTrip?.destination}...</p></div>)}
        {buildingManualRoute && (<div style={{ textAlign: 'center', padding: '60px 24px' }}><div className={styles.spinner} style={{ margin: '0 auto 20px' }}></div><p style={{ color: 'var(--muted)', fontSize: '15px' }}>Building your custom route...</p></div>)}
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px', flexWrap: 'wrap', gap: '12px' }}>
              ...
              <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
                ...
                <button onClick={saveItinerary} style={{ background: savedStatus === 'saved' ? 'rgba(76,217,100,0.15)' : 'rgba(255,255,255,0.06)', border: '1px solid ' + (savedStatus === 'saved' ? 'rgba(76,217,100,0.4)' : 'var(--border)'), color: savedStatus === 'saved' ? '#4cd964' : savedStatus === 'error' ? '#ff6b6b' : 'var(--muted)', borderRadius: '10px', padding: '8px 16px', fontSize: '13px', fontWeight: 600, cursor: 'pointer' }}>
                  {savedStatus === 'saving' ? `⏳ ${t('loading')}` : savedStatus === 'saved' ? `✓ ${t('save')}d!` : savedStatus === 'error' ? `❌ ${t('error')}` : `💾 ${t('save')}`}
                </button>
                <button onClick={() => { if (!user) { setGateReason('guest'); setShowGateModal(true); } else { saveAsPDF(); } }} style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid var(--border)', color: 'var(--muted)', borderRadius: '10px', padding: '8px 16px', fontSize: '13px', fontWeight: 600, cursor: 'pointer' }}>📄 PDF</button>
              ...
              return (
                <div style={{ marginBottom: '28px' }}>
                  <div style={{ display: 'flex', gap: '8px', marginBottom: '12px', flexWrap: 'wrap' }}>
                    <button onClick={() => setActiveMapDay(null)} style={{ background: activeMapDay === null ? 'var(--accent)' : 'rgba(255,255,255,0.06)', color: activeMapDay === null ? '#000' : 'var(--muted)', border: '1px solid var(--border)', borderRadius: '8px', padding: '6px 14px', fontSize: '12px', fontWeight: 700, cursor: 'pointer' }}>All {t('days')}</button>
                    {days.map(day => (<button key={day.day} onClick={() => setActiveMapDay(day.day)} style={{ background: activeMapDay === day.day ? DAY_COLORS[(day.day-1) % DAY_COLORS.length] : 'rgba(255,255,255,0.06)', color: activeMapDay === day.day ? '#000' : 'var(--muted)', border: '1px solid ' + (activeMapDay === day.day ? DAY_COLORS[(day.day-1) % DAY_COLORS.length] : 'var(--border)'), borderRadius: '8px', padding: '6px 14px', fontSize: '12px', fontWeight: 700, cursor: 'pointer' }}>Day {day.day}</button>))}
                  </div>
                  {(() => {
                    const filtered = activeMapDay === null ? allMarkers : allMarkers.filter(m => m.day === activeMapDay);
                    if (filtered.length === 0) return null;
                    return (
                      <div style={{ borderRadius: '16px', overflow: 'hidden', border: '2px solid ' + (activeMapDay === null ? 'var(--border)' : DAY_COLORS[(activeMapDay-1) % DAY_COLORS.length]), height: '360px' }}>
                        <MapContainer key={'map_' + activeMapDay} center={[filtered[0].lat, filtered[0].lng]} zoom={13} style={{ height: '100%', width: '100%' }} scrollWheelZoom={false}>
                          ...
                          {filtered.map((m, i) => (<Marker key={i} position={[m.lat, m.lng]} icon={NumberedIcon(m.order || i+1, m.day || 1)}><Popup><strong>Stop {m.order}: {m.name}</strong><br /><span style={{ fontSize: '11px', color: '#666' }}>{m.address}</span><br />{m.mapsUrl && <a href={m.mapsUrl} target="_blank" rel="noopener noreferrer" style={{ fontSize: '11px' }}>Open in Maps</a>}</Popup></Marker>))}
                        </MapContainer>
                      </div>
                    );
                  })()}
