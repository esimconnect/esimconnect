          ...
          <div style={{ maxWidth: '760px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px', flexWrap: 'wrap', gap: '12px' }}>
              ...
              <div style={{ display: 'flex', gap: '10px' }}>
                ...
                <button onClick={() => { if (!user && parseInt(localStorage.getItem('esim_iti_count') || '0') >= 2) { setGateReason('guest'); setShowGateModal(true); } else { saveAsPDF(); } }} style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid var(--border)', color: 'var(--muted)', borderRadius: '10px', padding: '8px 16px', fontSize: '13px', fontWeight: 600, cursor: 'pointer' }}>📄 Save PDF</button>
                <button onClick={() => { if (!user && parseInt(localStorage.getItem('esim_iti_count') || '0') >= 2) { setGateReason('guest'); setShowGateModal(true); } else { const text = routedPlan.days.map(d => 'Day ' + d.day + ': ' + d.theme + '\n' + (d.stops || []).map(s => s.order + '. ' + s.name + ' - ' + s.address).join('\n')).join('\n\n'); navigator.clipboard.writeText(text).then(() => alert('Itinerary copied!')); } }} style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid var(--border)', color: 'var(--muted)', borderRadius: '10px', padding: '8px 16px', fontSize: '13px', fontWeight: 600, cursor: 'pointer' }}>📋 Copy</button>
